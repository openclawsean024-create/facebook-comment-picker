...
export default function App() {
  ...
  const [scrapeSource, setScrapeSource] = useState('graph-api');
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await fetch(`/api/fetch-comments?url=${encodeURIComponent(postUrl.trim())}&scrapeSource=${scrapeSource}&rapidApiKey=${encodeURIComponent(rapidApiKey)}`);
      ...
      setFetchMeta(`已抓到 ${data.extractedCount} 筆可公開取得留言（${scrapeSource === 'rapid-api' ? 'RapidAPI' : 'Graph API'}）；貼文頁顯示 ${data.commentCountText || '未知'}。${data.note || ''}`);
    } catch (error) {
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="mt-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setScrapeSource('graph-api')} className={`flex-1 rounded-full py-2 px-3 text-xs font-bold transition-all ${scrapeSource === 'graph-api' ? 'bg-[#1877f2] text-white' : 'bg-[#f0f2f5] text-[#1877f2]/60'}`}>Graph API</button>
                <button onClick={() => setScrapeSource('rapid-api')} className={`flex-1 rounded-full py-2 px-3 text-xs font-bold transition-all ${scrapeSource === 'rapid-api' ? 'bg-[#ff6b35] text-white' : 'bg-[#f0f2f5] text-[#ff6b35]/60'}`}>RapidAPI 抓取</button>
              </div>
              {scrapeSource === 'rapid-api' && (
                <div className="mt-3">
                  <label className="mb-2 block text-sm font-bold text-[#ff6b35]">RapidAPI Key</label>
                  <input className="input-ui" type="password" value={rapidApiKey} onChange={(e) => setRapidApiKey(e.target.value)} placeholder="您的 RapidAPI Key" />
